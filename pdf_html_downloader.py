            ...
            try:
                if ".pdf" in self.ip_df.iloc[i]['ASSET_URL'].lower() or "download" in self.ip_df.iloc[i]['ASSET_URL'].lower():
                    ...
                    try:
                        ...
                        self.df = self.df.append([[self.ip_df.iloc[i]['ASSET_CODE'], "not working", "not working",
                                                   "not working", "not working","not working"]])

            except:
                print(self.ip_df.iloc[i]['ASSET_URL'])
                self.df = self.df.append([[self.ip_df.iloc[i]['ASSET_CODE'], "not working", "not working", "not working", "not working","not working"]])
        return self

    def pen_it(self):
        self.df.columns = ['asset_name','asset location','first page text','last page text','commissioned?',"comp_name"]
...
if __name__ == "__main__":
    ...
    df = pd.read_excel("c:\\garbage\\input_file.xlsx")
    extracting_comp("C:\\Users\\KarthickK\\Box\\VLRC Report\\2021Q3\\",df).downloading_pdf().pen_it()
